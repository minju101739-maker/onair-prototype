...
CREATE TYPE notification_kind_t AS ENUM ('meeting_confirmed','proposal_exhausted','approval_requested',
                                         'approval_decided','record_wrapped','integration_connected');
```
...
`notification_kind_t` → 설정 3분류 매핑: `meeting_confirmed→confirm`, `approval_*·proposal_exhausted→progress`, `record_wrapped→wrap`, `integration_connected`은 무분류(항상 통지).
